...
SRC_URI = "\
    ...
    ${@'file://platform-pre-switchroot.sh' if 'cryptparts' in d.getVar('MACHINEOVERRIDES').split(':') else ''} \
"
...
RDEPENDS_${PN} = "\
    ...
    ${@'luks-srv-app cryptsetup' if 'cryptparts' in d.getVar('MACHINEOVERRIDES').split(':') else ''} \
"
FILES_${PN} = "/"
